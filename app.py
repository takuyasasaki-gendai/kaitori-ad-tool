...
<table class="logic-table">
    ...
    <tr><td><b>② 見出し(15案)</b></td><td>独自の強み（USP）から、検索意図に刺さる「ベネフィット」を抽出し30文字以内に構成します。</td></tr>
    <tr><td><b>③ 説明文(4案)</b></td><td>見出しを補完しユーザーの不安を解消する「詳細な特徴」を90文字に文章化します。</td></tr>
    <tr><td><b>④ キーワード(20案)</b></td><td>獲得効率の高い組み合わせを、マッチタイプ別に戦略的に選定します。</td></tr>
...
def generate_ad_plan(site_text, api_key):
    try:
        ...
        【キーワード(④)】20個。Detailsにマッチタイプ、Other1に理由。「ターゲットキーワード」禁止。
        【⑤スニペット・⑥コールアウト】
        - Snippet: Contentに値、Detailsにヘッダーの種類(ブランド/種類等)を記述。
        - Callout: Contentにテキスト、Detailsに具体的な訴求メリットを記述。
        - 「〇文字以内」という注釈はDetailsに入れないこと。
        出力構成: 1. サイト分析（①強み ②課題 ③改善案） 2. [DATA_START] CSV出力 [DATA_END]
        サイト内容: {site_text}
        """
...
if st.session_state.ad_result:
    ...
    # CSVの目次などを消去
    cleaned_analysis = re.split(r'\n\s*(#{1,4}\s*[23]\.|\[DATA_START\])', analysis_raw)[0].strip()
